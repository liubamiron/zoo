...
function AdminAnimalsDetails() {
    ...
    useEffect(() => {
        const getAnimalData = async () => {
            try {
                const data = await fetchAnimalDataById(id); // Fetch the animal data by ID
                ...
                setDescrShortRO(data?.descr_short_ro || '');
                setDescrShortRU(data?.descr_short_ru || '');
                setDescrShortEN(data?.descr_short_en || '');

                setDescrLongRO(data?.descr_long_ro || '');
                setDescrLongRU(data?.descr_long_ru || '');
                setDescrLongEN(data?.descr_long_en || '');
                setHabitatRO(data?.habitat_ro || '');
                setHabitatRU(data?.habitat_ru || '');
                setHabitatEN(data?.habitat_en || '');

                // Family fields
                setFamilyRO(data?.family_ro || '');
                setFamilyRU(data?.family_ru || '');
                setFamilyEN(data?.family_en || '');

// Genus fields
                setGenusRO(data?.genus_ro || '');
                setGenusRU(data?.genus_ru || '');
                setGenusEN(data?.genus_en || '');

// Phylum fields
                setPhylumRO(data?.phylum_ro || '');
                setPhylumRU(data?.phylum_ru || '');
                setPhylumEN(data?.phylum_en || '');

// Class fields
                setClassRO(data?.clas_ro || '');
                setClassRU(data?.clas_ru || '');
                setClassEN(data?.clas_en || '');

// Domain fields
                setDomainRO(data?.domain_ro || '');
                setDomainRU(data?.domain_ru || '');
                setDomainEN(data?.domain_en || '');

// Karyotype fields
                setKaryotypeRO(data?.karyotype_ro || '');
                setKaryotypeRU(data?.karyotype_ru || '');
                setKaryotypeEN(data?.karyotype_en || '');

// Habitat long fields
                setHabitatLongRO(data?.habitat_long_ro || '');
                setHabitatLongRU(data?.habitat_long_ru || '');
                setHabitatLongEN(data?.habitat_long_en || '');

// General info fields
                setGeneralInfoRO(data?.general_info_ro || '');
                setGeneralInfoRU(data?.general_info_ru || '');
                setGeneralInfoEN(data?.general_info_en || '');

// Nutrition fields
                setNutritionRO(data?.nutrition_ro || '');
                setNutritionRU(data?.nutrition_ru || '');
                setNutritionEN(data?.nutrition_en || '');

// Facts fields
                setFactsRO(data?.facts_ro || '');
                setFactsRU(data?.facts_ru || '');
                setFactsEN(data?.facts_en || '');

                setProtectionRO(data?.protection_ro || '');
